...
def main(req: func.HttpRequest) -> func.HttpResponse:
	...
	if req.params.get("id") is not None:
		identity = req.params.get("id")
		try:
			base_url = ("{0}v1/offers/{1}?identifier={2}&timestamp={3}").format(resultDict["km_api_url_sandbox"].value, identity, resultDict["km_api_identifier_sandbox"].value, epochtime)
			print(base_url)
			...
			result = connection.call_km_api(method_in=method, signed_url_in=signed_url, body_in=req.get_json())
